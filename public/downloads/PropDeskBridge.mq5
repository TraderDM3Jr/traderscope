//+------------------------------------------------------------------+
//|                                          PropDeskBridge.mq5      |
//|  Bridge that pushes live account + position data from MT5 to     |
//|  the PropDesk dashboard (POST /api/ingest every N seconds).      |
//|                                                                  |
//|  Setup:                                                          |
//|  1) Copy this file to MQL5/Experts/PropDeskBridge.mq5 and        |
//|     compile in MetaEditor (F7).                                  |
//|  2) In MT5: Tools > Options > Expert Advisors > tick             |
//|     "Allow WebRequest for listed URL" and add your dashboard     |
//|     base URL, e.g. https://your-preview-url.e2b.app              |
//|  3) Attach to any chart; data posts every PushSeconds seconds.   |
//+------------------------------------------------------------------+
#property copyright "PropDesk"
...
input int    HistoryLookback = 60;   // seconds
...
void OnTick() { /* timer handles pushes */ }
...
string BuildBody()
{
  ...
  // --- Recently closed deals (optional) ---
  string closed = "";
  if (IncludeHistory)
  {
